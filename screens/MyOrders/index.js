...
export default function UserOrders ({ navigation, route }) {
  ...
  useEffect(() => {
    _getMyOrders(`?status=Pending&user=${user?.id}`)
    _getMyOrders(`?status=Completed&user=${user?.id}`)
  }, [])
  ...
  const totalAmount = () => {
    const list = active === 0 ? myOrders : myOrdersCompleted
    let total = 0
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.mainBody}>
        ...
        <FlatList
          data={active === 0 ? myOrders : myOrdersCompleted}
          key={'_'}
